...
/**
 * @author 015422404 给定一个头结点为 root 的链表, 编写一个函数以将链表分隔为 k 个连续的部分。
 * 
 ...
 *         \root.next.next.val = 3, 且 root.next.next.next = null。 第一个输出
 *         output[0] 是 output[0].val = 1, output[0].next = null。 最后一个元素
 *         output[4] 为 null, 它代表了最后一个部分为空链表。 示例 2：
 * 
...
public class SplitLinkedListinParts725 {
	...
	public static void main(String[] args) {
		...
		ListNode path1 = path;
		for (int i = 1; i < 8; i++) {
			path.next = new ListNode(i);
			path = path.next;
		}
		SplitLinkedListinParts725.splitListToParts(path1.next, 3);
	}
