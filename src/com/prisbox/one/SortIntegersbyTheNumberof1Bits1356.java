...
/**
 * @author LO 给你一个整数数组 arr 。请你将数组中的元素按照其二进制表示中数字 1 的数目升序排序。
 * 
 ...
 *         的个数排序得到的结果数组为 [0,1,2,4,8,3,5,6,7] 示例 2：
 * 
...
public class SortIntegersbyTheNumberof1Bits1356 {
	public int[] sortByBits(int[] arr) {
		Integer[] in = new Integer[arr.length];
		for (int i = 0; i < arr.length; i++) {
			in[i] = arr[i];
		}
		Arrays.sort(in, (n1, n2) -> {
			return Integer.bitCount(n1) == Integer.bitCount(n2) ? n1 - n2
					: Integer.bitCount(n1) - Integer.bitCount(n2);
		});
		for (int i = 0; i < arr.length; i++) {
			arr[i] = in[i];
		}
